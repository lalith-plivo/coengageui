...
import: "import { useForm } from 'coengageui_form';"
...
`coengageui_form` package does not depend on any other libraries, you can use it with or without `coengageui_core` inputs:
...
`setFieldValue` can be used with all `coengageui_core` inputs, example with [TextInput](/core/text-input/) component:
...
// value – input value
// onChange – input onChange function, works with all CoengageUI and regular inputs
...
// onChange – input onChange event, works with CoengageUI Checkbox, Switch and regular input[type="checkbox"]
// error – error message if input is invalid
```
