...
import { Slider as SliderStylesApi, Button as ButtonStylesApi } from '@coengageui/styles-api';

# Styles API

All CoengageUI components support styling of each component part with inline styles or by passing classes.
...
```tsx
import { Slider, createStyles } from '@coengageui/core';
...
## Styles API with CoengageUIProvider

You can also use Styles API in CoengageUIProvider with `styles` prop.
