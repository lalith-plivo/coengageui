...
import { MediaQueryDemos, CreateStylesDemos } from 'coengageui_demos';

# Responsive styles

`coengageui_core` package exports several components that can help you organize responsive styles without writing CSS.
...
`theme.breakpoints` are used in all responsive CoengageUI components. Breakpoints are set in px (rem, em and other values are not supported).
You can configure these values with [CoengageUIProvider](/theming/coengageui-provider/):
...
Most of CoengageUI components support `size` prop with `xs`, `sm`, `md`, `lg` and `xl` values.
...
All CoengageUI components support `sx` prop with which you can add styles (including responsive styles) to root component element:
...
If you want to add styles with `sx` prop to non-CoengageUI component, you can wrap it in [Box](/core/box/):
