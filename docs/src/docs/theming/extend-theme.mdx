...
By default, CoengageUI uses system fonts. You can change fonts and other text styles for headings, code and all other components:
...
CoengageUI uses [open-color](https://yeun.github.io/open-color/) in default theme with some additions.
...
CoengageUI supports 5 sizes for spacing, radius and shadows: xs, sm, md, lg, xl.
...
```tsx
import { Button, Code, CoengageUIProvider, Title } from 'coengageui_core';
...
CoengageUI supports 5 sizes for breakpoints (`xs`, `sm`, `md`, `lg`, `xl`), which are used in [Grid](/core/grid/), [SimpleGrid](/core/simple-grid/) and other components.
...
CoengageUI uses Bootstrap's breakpoint values as defaults:
...
```tsx
import { CoengageUIProvider, Box } from 'coengageui_core';
...
TypeScript will only autocomplete CoengageUI's default colors when accessing the theme. To add your custom colors to the CoengageUIColor type, you can use TypeScript module declaration:

```ts
import { Tuple } from 'coengageui_core';

type CustomColors = 'primaryColorName' | 'secondaryColorName';

declare module 'coengageui_core' {
  export interface CoengageUIThemeColorsOverride {
    colors: Record<CustomColors, Tuple<string, 10>>;
  }
}

// or if you want to "extend" standard colors
import { Tuple, DefaultCoengageUIColor } from 'coengageui_core';

type ExtendedCustomColors = 'primaryColorName' | 'secondaryColorName' | DefaultCoengageUIColor;
...
```ts
declare module 'coengageui_core' {
  export interface CoengageUIThemeOther {
