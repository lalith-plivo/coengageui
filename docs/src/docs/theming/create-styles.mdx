...
import { TabsDemos, CreateStylesDemos } from 'coengageui_demos';
...
## Using createStyles without coengageui_core
...
After installation you can import `createStyles` and other function from `coengageui_styles` instead of `coengageui_core` and use it separately:

```tsx
import { createStyles, CoengageUIProvider, useCoengageUITheme } from 'coengageui_styles';
```
