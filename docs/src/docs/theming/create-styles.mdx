...
import { TabsDemos, CreateStylesDemos } from '@coengageui/demos';

# createStyles

All CoengageUI components are built with [emotion](https://emotion.sh/) based css-in-js library.
We recommend using `createStyles` to create styles for the rest of your application
as it provides the most convenient way to utilize CoengageUI theme, but it is not required
...
defined via [CoengageUIProvider](/theming/extend-theme/#breakpoints) or just static values:
...
Most of CoengageUI components support [Styles API](/theming/styles-api/).
You can override default component styles inline or using [CoengageUIProvider](/theming/coengageui-provider/)
the same way as with `createStyles`:

<Demo data={TabsDemos.stylesApi} />

## Using createStyles without @coengageui/core

You can use `createStyles` and `CoengageUIProvider` without any other CoengageUI packages:
...
After installation you can import `createStyles` and other function from `@coengageui/styles` instead of `@coengageui/core` and use it separately:

```tsx
import { createStyles, CoengageUIProvider, useCoengageUITheme } from '@coengageui/styles';
```

## Can I use other css-in-js libraries or SCSS with CoengageUI?

Yes, you can. But please note that CoengageUI already uses [emotion](https://emotion.sh/) and you may
