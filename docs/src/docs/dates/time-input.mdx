---
group: 'coengageui-dates'
package: '@coengageui/dates'
title: TimeInput
order: 1
slug: /dates/time-input/
description: 'Capture time or time range from user'
props: ['TimeInput', 'TimeRangeInput']
import: "import { TimeInput, TimeRangeInput } from '@coengageui/dates';"
source: 'coengageui-dates/src/components/TimeInput/TimeInput.tsx'
docs: 'dates/TimeInput.mdx'
styles: ['TimeInput', 'TimeRangeInput']
---

import { TimeInputDemos, TimeRangeInputDemos } from '@coengageui/demos';

## Usage

TimeInput component:

<Demo data={TimeInputDemos.usage} />

TimeRangeInput component (supports the same props as TimeInput component):

<Demo data={TimeRangeInputDemos.usage} />

## Controlled

```tsx
import { useState } from 'react';
import { TimeInput } from '@coengageui/dates';

function Demo() {
  const [value, onChange] = useState(new Date());
  return <TimeInput value={value} onChange={onChange} />;
}
```

## With seconds

To add seconds set `withSeconds` prop:

<Demo data={TimeInputDemos.withSeconds} />

## 12 hours format

<Demo data={TimeInputDemos.format} />

## Input props

Component supports all props from [Input](/core/input/) and [InputWrapper](/core/input-wrapper/) components:

<Demo data={TimeInputDemos.configurator} />

## With icon

You can use any React node as icon:

<Demo data={TimeInputDemos.icon} />

## Invalid state and error

<Demo data={TimeInputDemos.validation} />

## Disabled state

<Demo data={TimeInputDemos.disabled} />

## Get input ref

You can get hours input ref with `ref` prop:

```tsx
import { useRef } from 'react';
import { TimeInput } from '@coengageui/dates';

function Demo() {
  const inputRef = useRef<HTMLInputElement>();
  return <TimeInput ref={inputRef} />;
}
```

## Accessibility

Provide hours, minutes and seconds labels to make inputs visible to screen reader:

```tsx
<TimeInput hoursLabel="Hours" minutesLabel="Minutes" secondsLabel="Seconds" />
```
