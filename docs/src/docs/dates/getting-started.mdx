---
group: 'coengageui-dates'
package: 'coengageui_dates'
title: 'Getting started'
pageTitle: 'CoengageUI Dates'
description: 'Capture date inputs from user and display calendars'
order: 0
slug: /dates/getting-started/
installation: 'coengageui_dates'
source: 'coengageui-dates/'
license: MIT
---

import {
  CalendarDemos,
  RangeCalendarDemos,
  MonthDemos,
  DatePickerDemos,
  TimeInputDemos,
  DateRangePickerDemos,
} from 'coengageui_demos';

## Installation

Package depends on:

- [react](https://www.npmjs.com/package/react)
- [react-dom](https://www.npmjs.com/package/react-dom)
- [dayjs](https://www.npmjs.com/package/dayjs)
- [coengageui_hooks](https://www.npmjs.com/package/coengageui_hooks)
- [coengageui_core](https://www.npmjs.com/package/coengageui_core)

Install with npm:

```sh
npm install dayjs coengageui_core coengageui_hooks coengageui_dates
```

Install with yarn:

```sh
yarn add dayjs coengageui_core coengageui_hooks coengageui_dates
```

## Localization

CoengageUI dates uses [dayjs](https://day.js.org/) library for localization.
Follow [example](https://day.js.org/docs/en/i18n/loading-into-nodejs) to load required locales in your application:

```tsx
// Import German language
import 'dayjs/locale/de';
```

[CoengageUIProvider](/theming/coengageui-provider/) allows you to set a `datesLocale` property for all date components, or you can overwrite the locale on a per component basis:

```tsx
import { DatePicker, Calendar, Month } from 'coengageui_dates';

function Demo() {
  return (
    <>
      <DatePicker locale="de" />
      <Calendar locale="de" />
      <Month locale="de" />
    </>
  );
}
```

## Components

[DatePicker](/dates/date-picker/)

<Demo data={DatePickerDemos.usage} demoProps={{ toggle: true, zIndex: 5 }} />

<Demo data={DateRangePickerDemos.usage} demoProps={{ toggle: true, zIndex: 4 }} />

[TimeInput](/dates/time-input/)

<Demo data={TimeInputDemos.usage} demoProps={{ toggle: true }} />

[Calendar](/dates/calendar/)

<Demo data={CalendarDemos.usage} demoProps={{ toggle: true }} />

<Demo data={RangeCalendarDemos.usage} demoProps={{ toggle: true }} />

[Month](/dates/month/)

<Demo data={MonthDemos.usage} demoProps={{ toggle: true }} />

## Utility functions

Apart from components coengageui_dates package also exports several utility functions.

**get-month-days**

Returns an array of weeks of current month:

```tsx
import { getMonthDays } from 'coengageui_dates';

getMonthDays(new Date()); // -> [[7xDate objects] x amount of weeks in current month]
```

**get-months-names**

Returns months names for given locale and format:

```tsx
import { getMonthsNames } from 'coengageui_dates';

getMonthsNames('en', 'MMMM');
// -> [ 'January', 'February', 'March', 'April', 'May', 'June', 'July', 'August', 'September', 'October', 'November', 'December']

// to localize import locale from dayjs package
import 'dayjs/locale/ru';
import { getMonthsNames } from 'coengageui_dates';

getMonthsNames('ru', 'MMMM');
// -> ['январь','февраль','март','апрель','май','июнь','июль','август','сентябрь','октябрь','ноябрь','декабрь']
```

**get-weekdays-names**

Returns an array of short weekdays names for given locale:

```tsx
import { getWeekdaysNames } from 'coengageui_dates';

getWeekdaysNames('en');
// -> ['Mo', 'Tu', 'We', 'Th', 'Fr', 'Sa', 'Su']
```

**get-years-range**

Returns an array of years, which fall into given range:

```tsx
import { getYearsRange } from 'coengageui_dates';

getYearsRange({ from: 2020, to: 2025 });
// -> [2020, 2021, 2022, 2023, 2024, 2025]
```

**is-same-date**

Returns true if two dates have the same year, month and date, other properties (minutes, seconds, etc.) are ignored:

```tsx
import { isSameDate } from 'coengageui_dates';

isSameDate(new Date(2020, 7, 21), new Date(2020, 7, 21)); // -> true
isSameDate(new Date(2020, 7, 21), new Date(2020, 7, 22)); // -> false
```

**is-same-month**

Returns true if two dates have the same year and month, other properties (date, minutes, seconds, etc.) are ignored:

```tsx
import { isSameMonth } from 'coengageui_dates';

isSameMonth(new Date(2020, 7, 21), new Date(2020, 7, 22)); // -> true
isSameMonth(new Date(2020, 7, 21), new Date(2020, 8, 22)); // -> false
```
