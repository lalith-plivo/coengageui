...
props: ['Input']
import: "import { Input } from 'coengageui_core';"
source: 'coengageui-core/src/components/Input/Input.tsx'
docs: 'core/Input.mdx'
styles: ['Input']
---

import { InputDemos } from 'coengageui_demos';
...
and Input component as base for your own custom inputs with CoengageUI theme.

<Demo data={InputDemos.configurator} />

## Variants

Input has 3 variants, all of which are available on all CoengageUI inputs.
...
If you want to add your own styles to input it's better to start from scratch rather than overriding CoengageUI styles,
use special `headless` variant which does not include any CoengageUI styles but still supports all other features: icon, right section, etc.:
