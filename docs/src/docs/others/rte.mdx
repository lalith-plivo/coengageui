...
import: "import { RichTextEditor } from 'coengageui_rte';"
source: 'coengageui-rte/src'
docs: 'others/rte.mdx'
installation: 'coengageui_rte'
license: MIT
...
Package has peer dependencies: [react](https://www.npmjs.com/package/react), [react-dom](https://www.npmjs.com/package/react-dom), [coengageui_hooks](https://www.npmjs.com/package/coengageui_hooks) and [coengageui_core](https://www.npmjs.com/package/coengageui_core):.

Install with npm:

```sh
npm install coengageui_rte coengageui_core coengageui_hooks
```

Install with yarn:

```sh
yarn add coengageui_rte coengageui_core coengageui_hooks
```
...
- `stickyOffset` – top property, used with sticky position, use it to offset elements with fixed position, for example, CoengageUI docs website has 60px header, in this case you should set stickyOffset to 60
...
import { RichTextEditor, Editor } from 'coengageui_rte';
...
export default dynamic(() => import('coengageui_rte'), {
