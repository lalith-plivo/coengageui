...
installation: 'coengageui_notifications'
license: MIT
---

import { NotificationDemos, NotificationsDemos } from 'coengageui_demos';
...
```sh
npm install coengageui_notifications coengageui_core coengageui_hooks
```

Install with yarn:

```sh
yarn add coengageui_notifications coengageui_core coengageui_hooks
```
...
`coengageui_notifications` package exports the following functions:
...
All functions can be imported from `coengageui_notifications` package and can be used in any part of your application:
...
```tsx
import { showNotification, hideNotification } from 'coengageui_notifications';
