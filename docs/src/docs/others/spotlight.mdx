...
import: "import { SpotlightProvider } from '@coengageui/spotlight';"
source: 'coengageui-spotlight/src'
docs: 'others/spotlight.mdx'
installation: '@coengageui/spotlight'
license: MIT
styles: ['SpotlightProvider']
---

import { SpotlightDemos } from '@coengageui/demos';

## Installation

Package depends on `@coengageui/core` and `@coengageui/hooks`.
...
`@coengageui/spotlight` exports several functions which can be used to perform certain actions
...
You can import `SpotlightAction` type from `@coengageui/spotlight` package:
