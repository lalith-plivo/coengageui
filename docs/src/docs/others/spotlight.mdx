...
import: "import { SpotlightProvider } from 'coengageui_spotlight';"
source: 'coengageui-spotlight/src'
docs: 'others/spotlight.mdx'
installation: 'coengageui_spotlight'
license: MIT
styles: ['SpotlightProvider']
---

import { SpotlightDemos } from 'coengageui_demos';
...
Note that if you are using [CoengageUIProvider](/theming/coengageui-provider/), `SpotlightProvider` must be used as its child:
...
You can setup multiple shortcuts, for example, CoengageUI documentation uses the following setup:
...
`coengageui_spotlight` exports several functions which can be used to perform certain actions
...
You can import `SpotlightAction` type from `coengageui_spotlight` package:
