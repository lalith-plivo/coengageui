...
import: "import { useQueue } from 'coengageui_hooks';"
docs: 'hooks/use-queue.mdx'
source: 'coengageui-hooks/src/use-queue/use-queue.ts'
---

import { NotificationsDemos } from 'coengageui_demos';

## Usage

use-queue limits the amount of data in current state and place the rest of it in a queue.
For example, in [coengageui_notifications](/others/notifications/) package amount of
...
```tsx
import { useQueue } from 'coengageui_hooks';
...
Example of use-queue hook usage in [CoengageUI notifications system](/others/notifications/). By default
