...
import: "import { useLocalStorage } from '@coengageui/hooks';"
docs: 'hooks/use-local-storage.mdx'
source: 'coengageui-hooks/src/use-local-storage/use-local-storage.ts'
---
...
```tsx
import { useLocalStorage } from '@coengageui/hooks';
...
CoengageUI docs website uses this hook to store color scheme information:

```tsx
import { useLocalStorage } from '@coengageui/hooks';
import { ActionIcon, ColorScheme } from '@coengageui/core';
...
You can test this feature by opening 2 tabs with CoengageUI docs side by side and changing color scheme
