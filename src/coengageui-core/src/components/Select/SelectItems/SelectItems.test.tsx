...
describe('coengageui_core/Select/SelectItems', () => {
  it('renders nothing found message if data array is empty', () => {
    ...
    expect(withData.querySelectorAll('.coengageui-Select-nothingFound')).toHaveLength(0);
    expect(withoutData.querySelector('.coengageui-Select-nothingFound').textContent).toBe(
      'test-nothing'
    );
  });
