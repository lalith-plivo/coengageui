...
export interface AppShellStylesParams {
  ...
  fixed: boolean;
  navbarOffsetBreakpoint: CoengageUINumberSize;
  asideOffsetBreakpoint: CoengageUINumberSize;
}

function getPositionStyles(props: AppShellStylesParams, theme: CoengageUITheme): CSSObject {
  ...
  return {
    minHeight: '100vh',
    paddingTop: `calc(var(--coengageui-header-height, 0px) + ${padding}px)`,
    paddingBottom: `calc(var(--coengageui-footer-height, 0px) + ${padding}px)`,
    paddingLeft: `calc(var(--coengageui-navbar-width, 0px) + ${padding}px)`,
    paddingRight: `calc(var(--coengageui-aside-width, 0px) + ${padding}px)`,
