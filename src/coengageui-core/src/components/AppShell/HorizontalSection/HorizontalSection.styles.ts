...
interface HorizontalSectionStyles {
  ...
  hiddenBreakpoint: CoengageUINumberSize;
  fixed: boolean;
  zIndex: number;
  ...
  (
    ...
    return {
      root: {
        ...theme.fn.fontStyles(),
        ...position,
        top: position?.top || 'var(--coengageui-header-height)',
        zIndex,
        height:
          height ||
          'calc(100vh - var(--coengageui-header-height, 0px) - var(--coengageui-footer-height, 0px))',
