...
describe('@coengageui/core/Menu', () => {
  ...
  itSupportsFocusEvents(Menu, defaultProps, '.coengageui-ActionIcon-root');
  itSupportsSystemProps({
    component: Menu,
    props: defaultProps,
  ...
  it('opens menu when controlled is clicked and trigger prop is "click"', async () => {
    ...
    expect(container.querySelector('.coengageui-Menu-body')).toBe(null);
    await actAsync(() => userEvent.click(screen.getByLabelText('test-menu')));
    expect(container.querySelector('.coengageui-Menu-body')).toBeInTheDocument();
  });
