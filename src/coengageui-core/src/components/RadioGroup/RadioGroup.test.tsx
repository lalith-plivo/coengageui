...
describe('coengageui_core/RadioGroup', () => {
  ...
  itFiltersChildren(RadioGroup, defaultProps, '.coengageui-RadioGroup-radio', [
    <Radio value="test-value-1" label="test-label-1" />,
    <Radio value="test-value-2" label="test-label-2" />,
  ]);
  ...
  it('passes correct name to Radio components', () => {
    ...
    expect(
      withoutName.querySelector('input[type="radio"]').getAttribute('name').includes('coengageui-')
    ).toBe(true);
  ...
  it('has an accessible label', () => {
    const radioGroup = render(
      <RadioGroup label="Select a Framework">
        <Radio value="coengage" label="CoengageUI" />
