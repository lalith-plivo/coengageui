...
function serializeJSON<T>(value: T) {
  try {
    ...
    throw new Error('coengageui_hooks use-local-storage: Failed to serialize the value');
  }
}
...
export function useLocalStorage<T = string>({
      ...
      if (val instanceof Function) {
        setValue((current) => {
          ...
          window.dispatchEvent(
            new CustomEvent('coengageui-local-storage', { detail: { key, value: val(current) } })
          );
          return result;
        });
      } else {
        ...
        window.dispatchEvent(
          new CustomEvent('coengageui-local-storage', { detail: { key, value: val } })
        );
        setValue(val);
      }
    },
  ...
  useWindowEvent('coengageui-local-storage', (event) => {
