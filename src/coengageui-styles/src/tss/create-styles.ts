import type { CoengageUITheme } from '../theme';
import type { CSSObject } from './types';
import { fromEntries } from './utils/from-entries/from-entries';
import { useCss } from './use-css';
import { useCoengageUITheme, useCoengageUIThemeStyles } from '../theme/CoengageUIProvider';
...
export function createStyles<Key extends string = string, Params = void>(
  ...
  function useStyles(params: Params, options?: UseStylesOptions<Key>) {
    ...
    const { styles: themeStyles, classNames: themeClassNames } = useCoengageUIThemeStyles(
      options?.name
    );
    ...
    function createRef(refName: string) {
      return `__coengageui-ref-${refName || ''}`;
    }
