...
export function itSupportsInputProps<P>(
  ...
  it('handles error and invalid state', async () => {
    ...
    expect(invalid.querySelectorAll(`.coengageui-${name}-error`)).toHaveLength(0);
    expect(withError.querySelector(`.coengageui-${name}-error`)).toBeInTheDocument();
    expect(withError.querySelector(`.coengageui-${name}-error`).textContent).toBe('Test error');
  });
